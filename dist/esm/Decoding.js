...
export async function decodeTransactions(signedTransactions, rootCertFingerprint) {
    return Promise.all(signedTransactions.map((transaction) => decodeJWS(transaction, rootCertFingerprint)));
}
export async function decodeTransaction(transaction, rootCertFingerprint) {
    return decodeJWS(transaction, rootCertFingerprint);
}
export async function decodeRenewalInfo(info, rootCertFingerprint) {
    return decodeJWS(info, rootCertFingerprint);
}
export async function decodeNotificationPayload(payload, rootCertFingerprint) {
 ...
 */
async function decodeJWS(token, rootCertFingerprint = APPLE_ROOT_CA_G3_FINGERPRINT) {
...
async function validateCertificates(certificates, rootCertFingerprint) {
    ...
    const x509certs = parseCertificate(stringToArrayBuffer(certificates.join("\n")));
